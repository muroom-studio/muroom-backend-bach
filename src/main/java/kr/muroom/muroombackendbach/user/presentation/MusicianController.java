...
public class MusicianController {
  ...
  public ApiResponse<MusicianSignUpResponse> registerMusician(
      @Valid @RequestBody MusicianSignUpDto musicianSignUpRequest, HttpServletRequest httpRequest) {
    ...
    MusicianSignUpResponse response =
        musicianService.registerMusician(musicianSignUpRequest, baseUrl);
  ...
  public ApiResponse<OAuthLoginResponse> oauthLogin(@RequestBody OAuthLoginRequest request,
    ...
    return ApiResponse.success(oAuthLoginService.login(request, baseUrl));
  }

  @Operation(summary = "내 간략 정보(프로필 이미지, 닉네임) 조회", description = "내 간략 정보(프로필 이미지, 닉네임)를 조회합니다.")
  @SecurityRequirement(name = "Bearer Authentication")
  @GetMapping("/me")
  public ApiResponse<MusicianDto.MusicianSimpleProfileResponse> getNickname(
      @AuthenticationPrincipal Long musicianId) {
    MusicianDto.MusicianSimpleProfileResponse response = musicianService.getMusicianSimpleProfile(
        musicianId);
