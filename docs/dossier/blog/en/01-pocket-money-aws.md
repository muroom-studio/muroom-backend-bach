...
The monthly bill draws the whole arc by itself: **$98** in November (console-clicked infra, RDS), climbing to a peak of **$236** in January as we added a dev environment the manual way, then dropping to a steady **~$150** the month I finished the Terraform migration and the RDS-to-EC2 move. That $86-per-month cliff *is* the optimization, visible in the ledger.
...
My favorite detail: the credential-rotation Lambda is *still* dutifully rotating the dev password every twenty-four hours, on a service nobody uses. Automation outlives intent.
