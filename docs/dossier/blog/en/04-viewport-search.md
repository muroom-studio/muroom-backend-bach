...
Here's the part that fails every checklist: `studios.location` has **no GiST index**. Every viewport query is a sequential scan.
...
The honest framing isn't "we forgot the index." It's that at our scale the index was premature, the fix is one line (`CREATE INDEX ... USING GIST (location)`), and I know exactly which threshold — thousands of studios, city-wide viewports — would trigger it. Growth roadmap, in order: GiST on `location`, B-trees on the `studio_id` join keys the filter subqueries hit, `pg_trgm` for the leading-wildcard keyword match, and collapsing seven `building_info` EXISTS subqueries into one join.

## What I'd defend and what I'd change

Defend: QueryDSL templates over native SQL (the filters compose; a 13-way dynamic native query is string soup), ALL-semantics for amenities, the sort whitelist, and measuring before indexing.

Change: the map-marker endpoint has no result cap — it trusts the client's zoom level to bound the viewport, which is a contract that should live in the server. And the displayed min-price and the price *sort* are computed by two independent implementations (Java fold vs SQL `COALESCE`), which can disagree on studios with partially-priced rooms. Nobody ever noticed — eight users — but I'd unify them first thing.
