...
When you leave RDS, the feature you miss isn't the dashboard — it's the boring hygiene that happens without you. Automated credential rotation is the one I refused to give up: a database password that never rotates is a password that eventually leaks into a laptop, a log, a screenshot.
...
- The rotation schedule is its own resource: we ran **dev every 1 day, prod every 7**. Dev-daily wasn't paranoia; it was a canary. If rotation breaks, I want to find out in dev within twenty-four hours, not in prod within a week.
