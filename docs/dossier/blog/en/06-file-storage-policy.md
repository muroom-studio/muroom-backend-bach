...
The beta shipped with the simplest thing that works: a single bucket, presigned PUT URLs (the browser uploads straight to S3; on `t4g`-class instances you do not want image bytes flowing through your JVM), and a service method that looped over filenames issuing URLs in batch. Content-type validation was a hardcoded whitelist inside the domain service.
...
- **A `temp/` prefix protocol.** Uploads land in `temp/`. When the owning entity is saved, the service *moves* each key to its permanent home; S3 has no move, so it's copy-then-delete. Anything still in `temp/` after seven days is garbage by definition, and an S3 lifecycle rule deletes it. Application cleanup and infrastructure cleanup are deliberately redundant; if a bug ever leaks a file, the bucket policy is the backstop.
...
V1's disease was interface growth. Public and private needs diverged, so methods split: `generatePresignedPutUrlForPublic`, `...ForPrivate`, validator callbacks injected per call site. Every new location (a drafts area, a report-evidence snapshot store) threatened two more methods. (Archaeological note: a javadoc example in this era mentions "trainer profile photos", a relic of reference code from an older side project. Code you copy carries its ghosts.)
...
The diff is my favorite part: **+236 / −268**. Eleven files, more capability, net negative code. When a refactor lands on the right abstraction instead of piling on another one, the diff tends to say so.

## What it cost

Two scars worth recording. First, somewhere between V0 and V2, the content-type whitelist quietly disappeared; the presigned URL now signs whatever type the client declares. I found this during a post-shutdown audit, not a review; my best forensics say it was lost in an AI-assisted rewrite, which is a lesson about reviewing the *deleted* lines of generated diffs even harder than the added ones. Second, S3 operations run inside DB transactions with no compensation, so a rollback after a `move` leaves storage and database briefly disagreeing. The `temp/` lifecycle rule bounds the damage, which is exactly why the redundant backstop earns its keep.

What stayed with me is simple. When a service interface grows a method per use-case, the use-cases are probably rows in a table you haven't written yet.
