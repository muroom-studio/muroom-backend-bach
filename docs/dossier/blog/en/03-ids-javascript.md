...
Our schema began life conventionally, with twenty-four PostgreSQL sequences, one per table, and `allocationSize = 1`, which means Hibernate made a round-trip to the database on every single INSERT just to ask "what number comes next?".
...
*That's the series. The service is gone; the invoices, the git history, and these three lessons are what's left. For a first startup, that's not a bad trade.*
